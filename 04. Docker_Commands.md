...
docker-compose up -d                           # Rebuild with existing build
...
### Clean and rebuild
```bash
# 1. Stop all containers and remove the network state
docker-compose down --remove-orphans

# 2. Clean up any corrupted image references (Safely)
docker image prune -f

# 3. Rebuild and start everything from scratch using the new dynamic config
# If your server supports it, try using 'docker compose' (v2) instead of 'docker-compose' (v1)
...
```
### Stop all Docker by Docker system level not dependent on the compose file
```
docker ps -a   # check running container list
docker stop CONTAINER_ID  # example f062fcb1b96e
