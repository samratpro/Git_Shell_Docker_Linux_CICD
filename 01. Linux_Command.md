...
### Permission
```bash
sudo chmod -R 777 /www/wwwroot/service_site
```
### Switch as root
```bash
sudo su -
...
# Failed on metricsoptimize.com/.user.ini → “Operation not permitted”.
lsattr /www/wwwroot/metricsoptimize.com/.user.ini
# ----i---------e------- /www/wwwroot/metricsoptimize.com/.user.ini  (i flag (immutable))
chattr -i /www/wwwroot/metricsoptimize.com/.user.ini
# remove flag then
chown www-data:www-data /www/wwwroot/metricsoptimize.com/.user.ini
chmod 644 /www/wwwroot/metricsoptimize.com/.user.ini
chown -R www-data:www-data /www/wwwroot
find /www/wwwroot -type f -exec chmod 644 {} \;
find /www/wwwroot -type d -exec chmod 755 {} \;

# quick check
find /www/wwwroot -type f ! -perm 644 -o -type d ! -perm 755
ls -ld /www/wwwroot /www/wwwroot/metricsoptimize.com
lsattr /www/wwwroot/metricsoptimize.com/.user.ini

# relock
chattr +i /www/wwwroot/metricsoptimize.com/.user.ini
```
### Check memory consumption of the first 20 process
```
ps aux --sort=-%mem | head -n 20
```
### Check port running and kill
```bash
sudo lsof -i :8000
# PID: 1485862, COMMAND: python3, PORT: 8000
sudo kill 1485862 / sudo kill -9 1485862
```
### Fix Broken
```bash
...
## Users Manage
```bash
sudo adduser username    # create user user
sudo usermod -aG docker username  # give docker permission
sudo su - username  # switch user
sudo deluser username sudo # delete user
```
