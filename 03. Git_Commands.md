...
```
## Merge conflight
quick solution in server
```bash
git fetch --all
git reset --hard origin/master
```

for django project run makemigrations to check conflict section
```bash
git status
# see Unmerged paths
# example
...
# [Press ENTER for all 3 prompts to leave them empty/default]

# 2. TROUBLESHOOTING (Only run if you get a "Permission Denied" error during generation)
sudo chown -R USER:USER ~/.ssh
sudo chmod 700 ~/.ssh

# 3. View and Copy the Public Key
cat ~/.ssh/id_ed25519.pub
# [Copy the entire string starting with ssh-ed25519]

# ========================================================
# 🌐 WEB INTERFACE STEPS (GITHUB)
# 1. Go to your GitHub Repo
# 2. Click on Settings tab
# 3. Click on Deploy Keys from the left sidebar
# 4. Click on Add deploy key button
# 5. Paste your copied key
# * Check "Allow write access" if you want to git push from the server.
# 6. Click Add key
# ========================================================

# 4. Set the remote URL to use SSH (Do NOT use the HTTPS token link!)
git remote set-url origin git@github.com:samratpro/repo.git

# 5. Push and Pull completely error-free
